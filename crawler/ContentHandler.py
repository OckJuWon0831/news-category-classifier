...
class ContentHandler:
    ...
    def file_article(self, html):
        ...
        if not page_type or page_type.attrs["content"] != "article":
            self.logger.info(
                f'Page is not of type "article". Actual type is: {page_type[:10] if page_type is str else None}'
            )
            return False

        try:

            self.news_article["date_published"] = (
                soup.find("meta", {"property": "article:published_time"})
                .attrs["content"]
                .strip()
            )

            self.news_article["headline"] = soup.find(
                "h1", {"data-editable": "headlineText"}
            ).text.strip()

            self.news_article["publisher"] = soup.find(
                "meta", {"property": "og:site_name"}
            ).attrs["content"]

            self.news_article["description"] = (
            ...
            self.news_article["keywords"] = (
                soup.find("meta", {"name": "keywords"}).attrs["content"].strip()
            )

        except Exception as err:
            is_article = False
            self.logger.info(f"Missing Article dataError: {err}")
            for k, v in self.news_article.items():
    ...
    def add_article(self):
        self.dataset = pd.concat(
            [self.dataset, pd.DataFrame([self.news_article])], ignore_index=True
        )
